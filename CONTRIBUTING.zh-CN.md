...
## structural parser 维护边界

`src/structural.ts` 已经不再是一个小型解析辅助模块。它实际上已经演化成了一个广义上的 parser VM / 状态机，
任何“大体等价但实现大改”的重写都很难安全评审。

- 涉及 `src/structural.ts` 的 Pull Request 应限制在 Bug 修复、正确性修复、以及范围很小的回归修复。
- 不要针对这个文件提交功能扩张、纯清理式重构、架构重写，或者“顺手简化 parser”的 PR，除非维护者事先明确要求。
- 如果修复必须修改 `src/structural.ts`，请尽量保持补丁最小，并附上复现方式或回归测试。

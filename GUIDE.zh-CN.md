...
- 写错的、未知的标签[自动降级为纯文本](#优雅降级)——不抛异常，不污染上下文
...
- [从零实现 link 标签](https://github.com/chiba233/yumeDSL/wiki/zh-CN-%E6%95%99%E7%A8%8B-link-%E6%A0%87%E7%AD%BE) — 从零到一个可用的 `$$link(url | text)$$`
- [游戏对话标签](https://github.com/chiba233/yumeDSL/wiki/zh-CN-%E6%95%99%E7%A8%8B-%E6%B8%B8%E6%88%8F%E5%AF%B9%E8%AF%9D) — 为视觉小说打字机构建 shake/color/wait 标签
- [安全 UGC 聊天](https://github.com/chiba233/yumeDSL/wiki/zh-CN-%E6%95%99%E7%A8%8B-%E5%AE%89%E5%85%A8UGC) — 白名单 inline 标签、屏蔽危险形式、处理错误
...
同一输入总是得到相同 ID，但在文档前面插入或删除内容会使后续所有 ID 发生偏移。

`createEasyStableId()` 返回一个**解析会话级、有状态的** `CreateId` 生成器。
每次调用创建一个独立的闭包和消歧计数器——ID 根据 token 内容而非流中位置生成。

### 稳定性保证

- **不同指纹**的编辑：稳定——不影响其他 ID。
- **相同指纹**的重复项按出现顺序消歧（`s-abc`、`s-abc-1`、…）。
  在已有重复项*前面*插入一个新的同指纹 token，会使后续重复项的后缀整体后移。
- 哈希为 FNV-1a 32-bit。不同指纹理论上可能碰撞到相同的 base key——
  计数器仍能保证唯一性，但不相关的 token 可能共享前缀。
  用于 UI keying 没有问题；不要用作数据库主键。
...
默认指纹使用 `type` + `value`。传入 `fingerprint` 函数可完全控制哪些信息参与哈希。

`TokenDraft` 有索引签名（`[key: string]: unknown`），handler 附加的字段可以访问但类型为 `unknown`——使用前需收窄：
...
> **每个导出的详细文档和示例请访问 [导出一览 wiki 页面](https://github.com/chiba233/yumeDSL/wiki/zh-CN-%E5%AF%BC%E5%87%BA%E4%B8%80%E8%A7%88)。**
...
| `createEasyStableId`  | `(options?: EasyStableIdOptions) => CreateId`           | 基于内容的[稳定 ID](#稳定-token-id)，支持自定义指纹 |
...
> **每个工具函数的详细文档和示例请访问 [处理器工具函数 wiki 页面](https://github.com/chiba233/yumeDSL/wiki/zh-CN-%E5%A4%84%E7%90%86%E5%99%A8%E5%B7%A5%E5%85%B7%E5%87%BD%E6%95%B0)。**
...
`TextToken[]` 的只读遍历和不可变树变换。

| 导出           | 签名                                                          | 描述             |
|--------------|-------------------------------------------------------------|----------------|
| `walkTokens` | `(tokens: TextToken[], visitor: WalkVisitor) => void`       | 前序深度优先遍历（纯副作用） |
| `mapTokens`  | `(tokens: TextToken[], visitor: MapVisitor) => TextToken[]` | 后序深度优先不可变变换    |

两者均向回调传递 `TokenVisitContext`（`{ parent, depth, index }`）。

**`WalkVisitor`** — 通用函数或 `Record<type, fn>` 按类型分发：
...
**`MapVisitor`** — 返回替换 token、数组（展开）或 `null`（删除）：
...
// 展开透明 wrapper——把子节点提升为兄弟节点
const unwrapped = mapTokens(tokens, (token) =>
    token.type === "wrapper" ? (token.value as TextToken[]) : token,
);
```

> 在 `mapTokens` 中，当前 token 本身按后序处理，但 `ctx.parent` 提供的是原始父节点上下文，
> 而不是一个已完成映射的父节点快照。不要依赖 `ctx.parent.value` 读取"已映射后的子节点"。
...
`DslContext` 打包了当前 parse 会话的 syntax 配置和 token id 生成器。
...
- **写自定义 handler 时** — 解析器会把 `ctx` 作为末尾参数传进来。把它透传给你调用的任何工具函数
  （`parsePipeArgs`、`createTextToken`、`unescapeInline`……），这样转义规则、管道分隔符和
  token ID 就和当前解析保持一致：

  ```ts
  link: {
      inline: (tokens, ctx) => {
          const args = parsePipeArgs(tokens, ctx);   // ← 同一套 syntax 和 id 生成器
          return {type: "link", url: args.text(0), value: args.materializedTailTokens(1)};
      },
  }
  ```

- **在解析器外调用工具函数时**（独立脚本、测试、后处理）— 手动构造一个：
...
- **只用 `parseRichText` 或 `createParser`，没有自定义 handler** — 完全不需要管 `DslContext`，
  解析器内部自己处理。

- **旧 handler 还没加 `ctx`** — 继续能跑（JS 会忽略多余参数），但管道拆分和反转义会回退到
  模块级默认值。如果你换了自定义 syntax，这些 handler 会静默用错分隔符。给 handler 加 `ctx`
  只需每个改一行，就能彻底避免这类 bug。
...
> **详细示例和优雅降级场景请访问 [错误处理 wiki 页面](https://github.com/chiba233/yumeDSL/wiki/zh-CN-%E9%94%99%E8%AF%AF%E5%A4%84%E7%90%86)。**

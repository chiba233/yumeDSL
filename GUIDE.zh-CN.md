...
零依赖、递归解析的富文本 DSL 解析器，支持可插拔的标签处理器、完全可配置的语法符号和自定义标签名规则。
...
**API 已稳定。** 所有公开 API 现已稳定，后续更新将保持向后兼容。
...
- **所有语法符号均可配置。** 前缀、分隔符、转义字符、block/raw 标记——通过 `options.syntax` 可覆盖任意或全部符号。标签名字符规则也可通过
  `options.tagName` 插拔。
...
- 处理器辅助函数，支持批量注册标签
- 完全可配置的语法（前缀、分隔符、转义字符、block/raw 标记）和标签名字符规则
- 一等结构化解析 API（`parseStructural`），与 `parseRichText` 共享同一套语言配置
- 未知标签优雅降级
- 自定义错误上报
- 管道参数和 token 处理工具函数
- 单遍前向扫描（无回溯）
...
`parseStructural` 是与 `parseRichText` 共享同一套语言配置（`handlers`、`allowForms`、`syntax`、`tagName`、`depthLimit`
）的一等结构化解析 API。
在输出树中保留标签形态（inline / raw / block），适用于**语法高亮、lint 和结构分析**。

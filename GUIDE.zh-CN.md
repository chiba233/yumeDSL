...
`createPipeHandlers` 的单形态简写。每个创建的 handler 保留原始 `arg`，按 pipe 拆出 `args`，并保留内容：
`{ type: tagName, arg, args, value: content }`。

新代码优先使用 `createPipeHandlers`，除非你就是想要更窄的单形态简写。
...
| 导出                                  | 说明                                               |
|-------------------------------------|--------------------------------------------------|
| `createPipeHandlers(definitions)`   | pipe-aware handler builder，inline/raw/block 任意组合 |

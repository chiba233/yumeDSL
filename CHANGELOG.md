...
- **Security / Performance: malformed unclosed-tag inputs are now parsed in linear time (fixes a quadratic-to-cubic worst case)**
  ...
  - **Trees are unchanged; the error stream is conservatively cleaner.** The produced node trees (structure **and** positions) are byte-for-byte identical to 1.5.1 across `parseStructural` / `parseRichText` / `stripRichText`, validated by a differential oracle (~27 000 generated cases per run on the **default syntax and a custom multi-character-token syntax**, plus deep-chain sweeps at depths 48–110 with mixed convert/close tails and adversarial red-team repros). The emitted **error set** satisfies `errors(1.5.2) ⊆ errors(1.5.1)` — 1.5.2 never invents a diagnostic — and the omitted keys are exclusively **rewind artifacts of 1.5.1's cascade**: (a) duplicate/position-shifted `DEPTH_LIMIT` re-reports from re-scanning the same tail at shifted depths, and (b) on depth-limit-crossing chains, `*_NOT_CLOSED` false positives that 1.5.1's *intermediate* rescans emitted against constructs that its own final parse result shows as properly closed/converted. Both classes were machine-classified: every omitted key vanishes from 1.5.1's own output when its `depthLimit` is set to effectively infinite (i.e. they are pure depth-rescan artifacts, not semantic diagnostics); zero genuine-key omissions across all oracle corpora. Additionally, on malformed inputs that interleave several unclosed episodes, the **order** of `*_NOT_CLOSED` relative to other diagnostics can differ from 1.5.1 (single-pass collect-and-flush vs. interleaved multi-pass replay); the set semantics and the `onError` contract matrix are unaffected.

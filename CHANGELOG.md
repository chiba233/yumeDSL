...
- **Security / Performance: malformed unclosed-tag inputs are now parsed in linear time (fixes a quadratic-to-cubic worst case)**
  ...
  - **Fixed without a rewind.** The EOF replay now flags the resumed root frame; on the (single) continued forward scan, an inline-form, inline-capable tag head that is *provably plain-dangling* is degraded straight to text instead of being re-pushed. Danglingness is decided by an explicit-stack, memoized **tail-resolution mini-scan** that models the parser's own `)$$` close / `)%` raw-convert / `)*` block-convert / depth-limit degrade **exactly** — so it stays correct where a cheaper "find the matching close" approximation would not (e.g. `$$a($$a(x)$$`, where the inner inline closes at EOF).
  - **Raw/block degradation chains linearized too.** With the inline rewind gone, unclosed `raw`/`block`-convert chains (`$$a($$r(x)%`, `$$b(x)*`) were still `O(n²)`: each re-scanned to EOF. A single controlled, lazy `O(n)` pass now records every whole-line `rawClose` / `blockClose` start position; `findRawClose` becomes an exact cached lookup, `findBlockClose` uses the table to short-circuit the no-close-ahead case, and `emitCloseNotFoundError` skips its per-line candidate scan when no `onError` handler is registered (it only fed `emitError`, a no-op without a handler). All malformed-degrade families now scale linearly, tracking the well-formed `O(n)` baseline.
  - **Output is unchanged.** The produced node trees (structure **and** positions) are byte-for-byte identical to 1.5.1 across `parseStructural` / `parseRichText` / `stripRichText`, validated by a ~40 000-case differential oracle run on the **default syntax and a custom multi-character-token syntax** (so any hardcoded token width would have surfaced as a divergence). One narrow, non-contract difference remains: on synthetic, deeply pathological inputs that interleave several independent malformed episodes, the **order** of emitted `*_NOT_CLOSED` errors relative to other diagnostics can differ from 1.5.1 — the **set** of errors is the same, and the `onError` contract matrix is unaffected.

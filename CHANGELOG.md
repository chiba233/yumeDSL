...
- **Incremental: fix `updateIncremental` tree diverging from a full reparse (incremental == full invariant)**
  - A property-based fuzz test (`tests/incrementalFuzz.test.ts`, random documents × random edit sequences) found edits after which `updateIncremental(...).tree` did not equal `parseStructural(newSource)` — a violation of the core incremental invariant. Root cause: the dirty window was parsed as an isolated slice, but yumeDSL's grammar has *unbounded* forward scans (raw/block/arg-close search, inline EOF finalization) that get silently truncated at the slice's right edge, so `reparseDirtyWindowUntilStable(...)`'s `reparsedEnd === dirtyEndNew` check (always true) could accept a window whose parse differed from the full document.
  - Four distinct classes are now fixed: (1) right-boundary forward-scan truncation — a new lookahead recorder (`internal/lookahead.ts`) flags when a scan reads to the window end and the window expands; (2) left-of-edit reused-zone staleness — zones whose parse read to the document end are flagged `readsPastEnd` and `findDirtyRange(...)` extends the dirty range left to include them; (3) end-of-document inserts not covered by the reparse window — the window now always covers the inserted text; (4) whole-line `*end$$` / `%end$$` closes at a mid-line window boundary — detected via the real `newSource` boundary char and the window expands.
  - Verified by the fuzz suite plus all existing incremental/perf-guard/version-behavior tests. **Known limitation:** a heavier fuzz run surfaces a deeper tail of pathological-input seam cases (reparse-vs-full text-segmentation at a reused-zone seam) that are not yet covered; the committed fuzz is bounded accordingly.
  - No public API change; behavior on well-formed edits is unchanged (constant-time guards only).

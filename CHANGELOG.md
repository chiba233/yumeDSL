...
- **Security / Performance: malformed unclosed-tag inputs are now parsed in linear time (fixes a quadratic-to-cubic worst case)**
  - The "no backtracking, worst-case `Θ(n)`" guarantee was **false** for a class of malformed inputs reachable through the **public API with default settings**. When a chain of unclosed inline tags dangled at end-of-input, the EOF recovery (`replayMalformedInlineChainAtEof`) rewound the cursor to the **outermost** open inline and re-entered the main loop; the resumed suffix was itself an unclosed chain, so recovery re-fired **once per nesting level** — a triangular `O(k²)`, and `O(n³)` together with the uncached depth-limit boundary skip. `$$a(x` repeated to fill ~10 KB took tens of seconds; larger inputs scaled cubically. This is a denial-of-service vector for any service that parses untrusted rich-text.
  - **Fixed without a rewind.** The EOF replay now flags the resumed parent frame (the document root, a `blockContent` frame, or an args frame — so unclosed chains *inside a closed block* are covered too); on the (single) continued forward scan, an inline-form, inline-capable tag head that is *provably dangling* is degraded straight to text instead of being re-pushed. Danglingness is decided by an explicit-stack, memoized **frame-fate walk** (`walkInlineFrameFate`) that mirrors the real inline frame's cursor semantics branch-for-branch by **calling the parser's own decision helpers** (`scanEndTagAt` close, `)%` / `)*` convert resume positions via the shared close caches, gating skips via `skipTagBoundaryWithCache`, arg-context escapes/separators) — a design chosen specifically because an earlier hand-rolled approximation drifted from the parser on depth-limit + convert-tail inputs. Frames at the depth limit are simulated by a **saturated-frame walk** whose per-position fate is a pure suffix property, back-filled along the whole scanned path (suffix sharing), so saturated scanning totals `O(n)` per parse; the overall walk is `O(n · depthLimit)` worst case, `O(n)` typical.
  - **Scoped fast path, byte-exact fallback.** The in-place peel is taken only when the walk also proves *error-key conservation* (nested heads on the dangle path must be re-processed by the later real rescan with identical emission keys; raw/block-form-dispatched heads must convert exactly at their `argClose` with a frame-free argument span; stateful shorthand heads are never walked past). When that guard fails, the head is pushed for real and recovery falls back to the byte-exact 1.5.1 replay cascade — correct but quadratic for those inputs. The reported DoS family (`$$a(x`…), convert-in-chain families (`$$a($$r(x)%`…), and in-block dangling chains all pass the guard and parse in linear time (measured ~×2 per size doubling up to 256 KB; 1.5.1 took tens of seconds at 10 KB).
  - **Raw/block degradation chains linearized too.** With the inline rewind gone, unclosed `raw`/`block`-convert chains (`$$a($$r(x)%`, `$$b(x)*`) were still `O(n²)`: each re-scanned to EOF. A single controlled, lazy `O(n)` pass now records every whole-line `rawClose` / `blockClose` start position; `findRawClose` becomes an exact cached lookup, `findBlockClose` uses the table to short-circuit the no-close-ahead case, and `emitCloseNotFoundError` skips its per-line candidate scan when no `onError` handler is registered (it only fed `emitError`, a no-op without a handler).
  - **Known remaining quadratic (pre-existing, out of scope here).** When a whole-line `blockClose` *does* exist ahead but `findBlockClose`'s nested depth-balancing scan cannot reach it (alternating shapes such as `$$a)*$$c(\n*end$$\n` repeated), each failed search still walks to EOF — `Θ(n²)` in 1.5.1 **and unchanged in 1.5.2**. This is a separate block-close-scan hot spot, independent of the inline-EOF recovery fixed here; documented in *Linear-Time Complexity* (Lemma C) and tracked as future work.
  - **Trees are unchanged; the error stream is conservatively cleaner.** The produced node trees (structure **and** positions) are byte-for-byte identical to 1.5.1 across `parseStructural` / `parseRichText` / `stripRichText`, validated by a differential oracle (~27 000 generated cases per run on the **default syntax and a custom multi-character-token syntax**, deep-chain sweeps at depths 48–110 with mixed convert/close tails, red-team repro packs, and an adversarial re-review including an exhaustive enumeration of >1 000 000 short token sequences plus ~1.6 M fuzz cases across 8 syntaxes × 13 option combos). The emitted **error set** satisfies `errors(1.5.2) ⊆ errors(1.5.1)` — 1.5.2 never invents a diagnostic — and across all of those corpora the only omitted keys are **duplicate/position-shifted `DEPTH_LIMIT` re-reports**, a rewind artifact of 1.5.1 re-scanning the same tail at shifted depths (machine-classified: every omitted key also vanishes from 1.5.1's own output at an effectively infinite `depthLimit`; zero non-`DEPTH_LIMIT` omissions). Non-`DEPTH_LIMIT` emissions are conserved *by construction*: the single-pass peel fires only when the fate walk proves the skipped frame's in-frame emissions get re-keyed by the continued real scan, and anything else falls back to the byte-exact 1.5.1 cascade — stateful shorthand heads; content-skipping head boundaries the rescan would traverse differently (e.g. gating-rejected inline-form heads); form-dispatched heads not converting exactly at their `argClose`; escape sequences the arg/root/blockContent token sets tokenize differently (overlap syntaxes such as `tagOpen` sharing a character with `tagPrefix`); multi-character `tagClose`/`tagDivider` consumed as text (the parent rescan advances per character); and depth-sensitive close/convert resolutions inside the saturated region (adjacent-depth rescans can flip which frame owns a close token, e.g. `rawOpen === tagClose` syntaxes). Additionally, on malformed inputs that interleave several unclosed episodes, the **order** of `*_NOT_CLOSED` relative to other diagnostics can differ from 1.5.1 (single-pass collect-and-flush vs. interleaved multi-pass replay); the set semantics and the `onError` contract matrix are unaffected.
  - **No `src/core/**` rule was bent.** No `indexOf` / `lastIndexOf` (all amortization is the project's controlled fill-from-position cache pattern), no hardcoded token lengths or length-based "is there a token ahead" inference (tokens stay fully user-configurable), no regex, no ambient/module-level state (per-parse closures + explicit parameter threading), and every traversal is explicit-stack.
  - **Tests**: existing `test:coverage` (501), version-behavior parity, `onerror-matrix`, `parser-contract`, and `incrementalFuzz` (50 k docs) all pass unchanged.
- No breaking public API changes

### 1.5.1

- **Incremental: `updateIncremental(...)` now provably re-converges to the full parse (signature-driven re-convergence)**
  - Fixes a class of latent correctness bugs where `updateIncremental(doc, edit, newSource).tree` (and `createIncrementalSession().applyEdit(...)`) could diverge from `parseStructural(newSource, ...)` — including node structure **and** positions. The previous "reparse the dirty slice in isolation + a fixed-width seam probe" reuse was unsound at several boundaries.
  - **Right side** now uses *signature-driven re-convergence*: the dirty window is re-parsed continuously from a provably-clean left anchor and the window is grown (doubling, budget-guarded) until a re-parsed zone re-converges with a cached old zone on *same new-coordinate offset boundary + same structural signature + non-truncated + cross-window stability*. This correctly handles unbounded forward scans truncated at slice boundaries, constructs bleeding rightward, end-of-document inserts, whole-line `*end$$` / `%end$$` mid-window boundaries, seam text-segment merges, and the parser's non-prefix-monotonic degradation (where appending content retroactively changes earlier nodes).
  - **Left side** is guarded by (a) an *args-overhang* check — a reused-left zone whose subtree reads past the edit (degenerate `findTagArgClose` failure producing an out-of-bounds arg text) forces a full rebuild; and (b) a *structural-token skeleton* comparison of the dirty region (covering `tagPrefix` / `tagOpen` / `tagClose` / `tagDivider` / `endTag` / `rawOpen` / `blockOpen` and whole-line `rawClose` / `blockClose` / escape, with overlapping-prefix–correct scanning) — if the edit changes the token skeleton (which is what enables a left construct's scan to reach into the dirty region, or a dirty leading construct to merge left), it falls back to a full rebuild.
  - **Lazy-shift position fix**: reused right-side zones are now always re-resolved against the new position tracker (even when the net offset delta is `0`). A same-length edit that changes a newline (e.g. `L` → `\n`) shifts every subsequent line/column, which the previous `delta === 0` fast path skipped, leaving stale `line` / `column` on reused nodes.
  - **No `src/core/**` changes** — the entire fix lives in `src/incremental/**`. No ambient core state, no forced `onError`, no fattened shared closures, no `try/finally` on hot paths. All tree traversals are **explicit-stack** (no recursion) — deep nesting cannot overflow the call stack, consistent with the rest of the library.
  - **Performance**: well-formed local edits stay incremental and bounded — reparse bytes are `O(edit)` and assembly is `O(Z)` (unchanged). Plain-text editing *below* an unclosed/degraded construct stays incremental (no `O(n)`-per-keystroke regression). Edits that change the dirty-region structural-token skeleton conservatively fall back to a full rebuild — incremental reuse is best-effort for performance and never trades away correctness.
  - **Tests**: added `tests/incrementalFuzz.test.ts` — a seeded, reproducible property-based fuzz asserting the `incremental == full` invariant (tree with positions + valid zone partition) across both the low-level and public session APIs, over deterministic regression sources (known failure classes + CRLF) and millions of random doc/edit applications (incl. custom syntax + shorthand). Two defensive `incremental.test.ts` cases were updated to reflect the finer-grained reuse (a corrupted *seam* zone still surfaces as `UNKNOWN`; a corrupted *tail* zone now surfaces lazily on materialization; an inconsistent cached right zone is re-parsed into the dirty window rather than triggering a whole-document rebuild).

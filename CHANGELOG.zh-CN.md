...
- **安全 / 性能：畸形未闭合输入现在以线性时间解析（修复二次到三次方的最坏情况）**
  - "不回溯、最坏 `Θ(n)`"的承诺对一类畸形输入是**假的**，而且经**公开 API + 默认配置**即可触发。当一串未闭合 inline 标签在文末悬挂时，EOF 恢复（`replayMalformedInlineChainAtEof`）把游标倒带回**最外层**那个未闭合 inline 再重入主循环；而被恢复的后缀本身又是一条未闭合链，于是恢复**按嵌套层数每层重放一次**——三角形的 `O(k²)`，叠加未缓存的 depth-limit 边界跳过后是 `O(n³)`。把 `$$a(x` 重复填到约 10 KB 就要几十秒，更大的输入按三次方膨胀。对任何解析不可信富文本的服务而言，这是一个拒绝服务（DoS）入口。
  - **无倒带修复。** EOF 重放现在给被恢复的父帧打标（文档根 / `blockContent` 帧 / args 帧——因此*闭合 block 内部*的未闭合链同样覆盖）；在那一遍（唯一的）继续向前扫描里，凡是 inline 形态、具备 inline 能力、且**可证明悬挂**的标签头，直接降级成文本，而不再重新入栈。是否悬挂由一个显式栈、带记忆化的**帧命运推演**（`walkInlineFrameFate`）判定：它逐分支镜像真实 inline 帧的 cursor 语义，且每条分支**直接调用解析器自身的判定函数**（`scanEndTagAt` 闭合、经共享 close 缓存的 `)%` / `)*` 转换续扫位、`skipTagBoundaryWithCache` 的 gating 跳过、args 语境的转义与分隔符）——特意如此设计，因为更早的一版手写近似在 depth-limit + 转换尾输入上与真解析器发生了漂移。深度到达 depthLimit 的帧由**饱和帧推演**模拟——其每个位置的命运是纯后缀性质，一次扫描即可回填整条路径（后缀共享），故饱和扫描每次解析合计 `O(n)`；整个推演最坏 `O(n · depthLimit)`、典型 `O(n)`。
  - **有范围的快路，逐字节等价的回退。** 只有当推演同时证明*错误键守恒*（悬挂路径上的嵌套头必须能被后续真实重扫以同发射键重新处理；会被按 raw/block form 分发的头必须恰在其 `argClose` 处以同形态转换、且参数区内无嵌套帧；带状态的 shorthand 头一律不越过）时才就地剥离。守恒闸不满足时，该头照常真实入栈，恢复退回 1.5.1 逐字节等价的 replay 级联——对那些输入正确但为平方级。上报的 DoS 家族（`$$a(x`…）、链中转换家族（`$$a($$r(x)%`…）与块内悬挂链都通过守恒闸、以线性时间解析（实测到 256 KB 每次尺寸翻倍约 ×2；1.5.1 在 10 KB 就要数十秒）。
  - **raw/block 降级链也一并线性化。** 去掉 inline 倒带后，未闭合的 `raw`/`block` 转换链（`$$a($$r(x)%`、`$$b(x)*`）仍是 `O(n²)`：每个都要扫到 EOF。现在一遍受控、惰性的 `O(n)` 扫描记下全文每个整行 `rawClose` / `blockClose` 的起始位置；`findRawClose` 变成精确的缓存查找，`findBlockClose` 用该表对"后方根本没有闭合符"的情形短路，`emitCloseNotFoundError` 在没有注册 `onError` 时跳过它那一遍逐行候选扫描（它本来只喂给 `emitError`，无 handler 时是 no-op）。
  - **已知残留的二次方（既有问题，不在本次范围）。** 当后方**存在**整行 `blockClose`、但 `findBlockClose` 的嵌套配平扫描到不了它时（如 `$$a)*$$c(\n*end$$\n` 重复的交替形态），每次失败搜索仍要走到 EOF——`Θ(n²)`，1.5.1 如此、**1.5.2 仍然如此**。这是 block close 扫描里一个独立的既有热点，与本次修复的 inline-EOF 恢复无关；已记录于《线性时间复杂度》（引理 C），作为后续工作跟踪。
  - **树不变；错误流保守地更干净。** 产出的节点树（结构**和** position）与 1.5.1 在 `parseStructural` / `parseRichText` / `stripRichText` 上逐字节一致，由差分 oracle 验证（每轮约 2.7 万生成用例，覆盖**默认语法和一套自定义多字符 token 语法**，另有深度 48–110、混合转换/闭合尾的深链扫描、红队复现包，以及一轮对抗性复审——含超过 100 万条短 token 序列的穷举与约 160 万条跨 8 套语法 × 13 组选项的 fuzz）。发出的**错误集合**满足 `errors(1.5.2) ⊆ errors(1.5.1)`——1.5.2 绝不发明诊断——且在上述全部语料中，被省略的键**仅有重复/移位重发的 `DEPTH_LIMIT`**——1.5.1 以平移深度重扫同一尾巴的倒带伪影（经机器分类验证：把 1.5.1 的 `depthLimit` 调到近乎无穷后，每一个被省略的键都从 1.5.1 自己的输出中消失；非 `DEPTH_LIMIT` 键省略为零）。非 `DEPTH_LIMIT` 发射的守恒是*构造性的*：单遍剥离只在推演证明「被跳过的那次真实扫描的帧内发射会被后续真实重扫同键重发」时启用，其余情形一律退回 1.5.1 逐字节等价的级联——带状态的 shorthand 头；重扫会以不同方式穿越的「跳过内容」头边界（如被 gating 拒绝的 inline 形态头）；不恰在 `argClose` 处同形态转换的 form 分发头；arg/root/blockContent 三种转义 token 集合分词不一致的转义序列（如 `tagOpen` 与 `tagPrefix` 共享字符的重叠语法）；被当作文本消费的多字符 `tagClose`/`tagDivider`（父帧重扫逐字符推进）；以及饱和区内深度敏感的闭合/转换归属（相邻深度重扫可翻转 close token 属于哪个帧，如 `rawOpen === tagClose` 的语法）。此外，在交错多段未闭合片段的畸形输入上，`*_NOT_CLOSED` 相对其它诊断的**顺序**可能与 1.5.1 不同（单遍收集统一发出 vs. 多遍交错重放）；集合语义与 `onError` 合约矩阵不受影响。
  - **没有破任何 `src/core/**` 的规矩。** 无 `indexOf` / `lastIndexOf`（所有摊还都走项目自己的受控 fill-from-position 缓存模式），不写死 token 长度、不用基于长度的"前方有没有 token"推断（token 保持完全可自定义），无正则，无 ambient/模块级状态（每次解析的闭包 + 显式穿参），且每个遍历都是显式栈。
  - **测试**：现有 `test:coverage`（501）、版本行为一致性、`onerror-matrix`、`parser-contract`、`incrementalFuzz`（5 万文档）全部照常通过。
- 无破坏性公开 API 变更
...
- **渲染器内部：文本合并和 raw 转义路径都做了收紧**
  - `renderNodes(...)` 新增 `RenderFrame.textBuf` / `textBufPosition`，并拆出 `bufferText(...)` / `flushTextBuf(...)` 两个辅助路径：
    ...
    - 遇到 inline / raw / block 或帧结束时再一次性 `join("")` 写回 token；
    - position 语义保持和旧版 `mergeTextToken` 一致，不改输出，只减少文本密集场景下逐段扩展字符串的中间开销。
  - `renderRawNode(...)` 的 raw-close 还原改成“惰性创建 `parts`”：
    - 先用单游标扫描 `escapeChar + rawClose`；
    - 真正命中第一处 escaped close 之前都不分配缓冲；
    - 如果整段 raw 都没命中，就直接复用原始 `rawContent`；
    - 这样大块 raw 内容在常见无命中路径上会少很多无谓分配。
- 无破坏性公共 API 变化
...
- **新增：`filterTokens(tokens, predicate)`**
  ...
  - 它的语义是“树裁剪”，不是“平铺筛选”：保留节点仍保持原有嵌套结构，被拒绝的子树会原位移除。
...
- **增量解析：损坏的深层 inline 编辑不再卡住**
  - 对深层 `$$tag(...)$$` 嵌套内容一次删除大量尾部 `)$$` 闭合标记时，incremental session update 不再落入极慢的 EOF 恢复路径。
  - 这类修复尤其针对编辑器场景：用户正在删除或重输闭合符时，源码会暂时处于不合法状态，但增量更新不应因此卡住。
- **EOF 恢复语义已在全量解析与增量重解析之间对齐**
  - 输入在 EOF 处存在未闭合 inline 时，full parse、dirty-window reparse 与 seam probe 现在使用同一套结构恢复语义，避免整文档路径与增量路径出现不必要分叉。
- **升级说明**
  - `1.4.1` 没有破坏性公共 API 变更。
  - 现有 `1.4.0` 集成代码可直接升级到 `1.4.1`。
...
- **增量 diff：新增显式 no-op 信号**
  - `TokenDiffResult` 新增 `isNoop`，调用方可以直接区分“有编辑事件但结构结果未变化”和“正常非空 diff”。
  - `isNoop` 语义明确：仅当不存在结构 token 变化（`patches` 与 `ops` 同时为空）时为 `true`。
- **保守 diff 契约补充**
  - 整树保守回退路径的 no-op 判定改为显式空树条件，减少下游对返回语义的歧义解读。
- **文档更新（中英文增量解析 wiki）**
  - 在 `TokenDiffResult` 字段说明与类型展开中补充 `isNoop`。
  - 增补编辑器集成提示：多段编辑/同帧合并场景下，避免“只保留最后一条 diff”覆盖策略，否则可能被后续 `isNoop` 结果遮蔽前一条有意义 diff。
- 无破坏性公共 API 变化

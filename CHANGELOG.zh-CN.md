...
- **安全 / 性能：畸形未闭合输入现在以线性时间解析（修复二次到三次方的最坏情况）**
  ...
  - **无倒带修复。** EOF 重放现在给被恢复的根帧打标；在那一遍（唯一的）继续向前扫描里，凡是 inline 形态、具备 inline 能力、且**可证明为纯悬挂**的标签头，直接降级成文本，而不再重新入栈。是否"纯悬挂"由一个显式栈、带记忆化的**尾部归结 mini 扫描**判定，它**精确**复刻解析器自身的 `)$$` 闭合 / `)%` 转 raw / `)*` 转 block / depth-limit 降级语义——因此在"找匹配闭合符"的廉价近似会出错的地方它依然正确（例如 `$$a($$a(x)$$`，内层 inline 恰在 EOF 处闭合）。
  - **raw/block 降级链也一并线性化。** 去掉 inline 倒带后，未闭合的 `raw`/`block` 转换链（`$$a($$r(x)%`、`$$b(x)*`）仍是 `O(n²)`：每个都要扫到 EOF。现在一遍受控、惰性的 `O(n)` 扫描记下全文每个整行 `rawClose` / `blockClose` 的起始位置；`findRawClose` 变成精确的缓存查找，`findBlockClose` 用该表对"后方根本没有闭合符"的情形短路，`emitCloseNotFoundError` 在没有注册 `onError` 时跳过它那一遍逐行候选扫描（它本来只喂给 `emitError`，无 handler 时是 no-op）。所有畸形降级族现在都线性伸缩，与良构输入的 `O(n)` 基线一致。
  - **输出不变。** 产出的节点树（结构**和** position）与 1.5.1 在 `parseStructural` / `parseRichText` / `stripRichText` 上逐字节一致，由一个约 4 万例的差分 oracle 在**默认语法和一套自定义多字符 token 语法**上验证（因此任何写死 token 宽度的假设都会暴露为差异）。仅剩一处狭窄的、非合约的差别：在那种交错了多段彼此独立畸形片段的、人造的极端病态输入上，发出的 `*_NOT_CLOSED` 错误相对其它诊断的**顺序**可能与 1.5.1 不同——错误的**集合**相同，且 `onError` 合约矩阵不受影响。

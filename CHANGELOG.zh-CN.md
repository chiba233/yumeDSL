...
- **安全 / 性能：畸形未闭合输入现在以线性时间解析（修复二次到三次方的最坏情况）**
  ...
  - **树不变；错误流保守地更干净。** 产出的节点树（结构**和** position）与 1.5.1 在 `parseStructural` / `parseRichText` / `stripRichText` 上逐字节一致，由差分 oracle 验证（每轮约 2.7 万生成用例，覆盖**默认语法和一套自定义多字符 token 语法**，另有深度 48–110、混合转换/闭合尾的深链扫描与对抗性红队复现）。发出的**错误集合**满足 `errors(1.5.2) ⊆ errors(1.5.1)`——1.5.2 绝不发明诊断——且被省略的键全部是 **1.5.1 级联的倒带伪影**：(a) 以平移深度重扫同一尾巴时重复/移位重发的 `DEPTH_LIMIT`；(b) 在跨 depth-limit 的链上，1.5.1 的*中间*重扫对「其自身最终解析结果显示为正常闭合/转换」的构造误发的 `*_NOT_CLOSED` 假阳性。两类都经机器分类验证：把 1.5.1 的 `depthLimit` 调到近乎无穷后，每一个被省略的键都会从 1.5.1 自己的输出中消失（即它们是纯深度重扫伪影，不是语义诊断）；全部 oracle 语料中真实键省略为零。此外，在交错多段未闭合片段的畸形输入上，`*_NOT_CLOSED` 相对其它诊断的**顺序**可能与 1.5.1 不同（单遍收集统一发出 vs. 多遍交错重放）；集合语义与 `onError` 合约矩阵不受影响。

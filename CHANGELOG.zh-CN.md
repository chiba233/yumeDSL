...
- **escape 行为按解析区域严格收口**
  - `root` 仅接受 `tagOpen`、`tagClose`、`endTag` 的转义。
  - `arg` 不再把 `rawClose` / `blockClose` 视为可转义 token。
  - `block` content 支持 `blockClose` 的转义，不会再因被转义的 block-end 提前闭合。
  - `raw` 闭合符转义在渲染路径上的既有行为保持不变。
- **转义与边界交互稳定性修复**
  - 改进了 block/raw 闭合边界与参数扫描中的转义一致性表现。

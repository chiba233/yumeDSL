import { findBlockClose, findMalformedWholeLineTokenCandidate, findRawClose, findTagArgClose, } from "./scanner.js";
import { getSyntax } from "./syntax.js";
import { consumeBlockTagTrailingLineBreak, normalizeBlockTagContent } from "./blockTagFormatting.js";
import { createToken } from "./createToken.js";
export const tryParseComplexTag = (text, tagOpenPos, tag, tagNameEnd, inlineEnd, depthLimit, mode, handlers, blockTagSet, parseInlineContent) => {
    ...
    if (isBlock) {
        ...
        if (end === -1) {
            const malformedCloseCandidate = findMalformedWholeLineTokenCandidate(text, contentStart, blockClose);
            return {
                handled: true,
        ...
        if (!handler.block) {
            return {
                handled: true,
                nextIndex: consumeBlockTagTrailingLineBreak(tag, text, end + blockClose.length, mode, blockTagSet),
                fallbackText: text.slice(tagOpenPos, end + blockClose.length),
            };
        }
        const arg = text.slice(tagNameEnd + 1, argClose).trim();
        const blockContent = normalizeBlockTagContent(tag, text.slice(contentStart, end), mode, blockTagSet);
        return {
            handled: true,
            nextIndex: consumeBlockTagTrailingLineBreak(tag, text, end + blockClose.length, mode, blockTagSet),
            token: createToken(handler.block(arg, parseInlineContent(blockContent, Math.max(depthLimit - 1, 0), { mode }))),
        };
    }
    // ── Raw ──
    ...
    if (end === -1) {
        const malformedCloseCandidate = findMalformedWholeLineTokenCandidate(text, contentStart, rawClose);
        return {
            handled: true,
            nextIndex: contentStart,
            fallbackText: text.slice(tagOpenPos, contentStart),
    ...
    if (!handler.raw) {
        return {
            handled: true,
            nextIndex: consumeBlockTagTrailingLineBreak(tag, text, end + rawClose.length, mode, blockTagSet),
            fallbackText: text.slice(tagOpenPos, end + rawClose.length),
        };
    }
    const arg = text.slice(tagNameEnd + 1, argClose).trim();
    const rawContent = text.slice(contentStart, end);
    const normalizedRawContent = mode === "highlight"
        ? rawContent

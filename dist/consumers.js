...
import { findBlockClose, findInlineClose, findRawClose, getTagCloserType, readTagStartInfo, skipDegradedInline, } from "./scanner.js";
import { tryParseComplexTag } from "./complex.js";
import { createToken } from "./createToken.js";
import { consumeBlockTagTrailingLineBreak } from "./blockTagFormatting.js";
export const tryConsumeDepthLimitedTag = (ctx, info) => {
    if (ctx.stack.length < ctx.depthLimit)
        return false;
    const { endTag, blockClose, blockOpen, rawClose, rawOpen } = getSyntax();
    if (ctx.stack.length === ctx.depthLimit) {
        emitError(ctx.onError, "DEPTH_LIMIT", ctx.text, ctx.i, info.inlineContentStart - info.tagOpenPos);
    }
    ...
    if (tagInfo.closer === rawClose) {
        ...
        ctx.i = consumeBlockTagTrailingLineBreak(info.tag, ctx.text, end + rawClose.length, ctx.mode, ctx.blockTagSet);
        return true;
    }
    return false;
};
export const tryConsumeComplexTag = (ctx, info, inlineEnd, parseInlineContent) => {
    const result = tryParseComplexTag(ctx.text, info.tagOpenPos, info.tag, info.tagNameEnd, inlineEnd, ctx.depthLimit, ctx.mode, ctx.handlers, ctx.blockTagSet, parseInlineContent);
    if (!result.handled)
        return false;
    if (result.error) {
...
export const tryConsumeInlineTag = (ctx, info, inlineEnd) => {
    if (inlineEnd === -1) {
        emitError(ctx.onError, "INLINE_NOT_CLOSED", ctx.text, info.tagOpenPos, info.inlineContentStart - info.tagOpenPos);
        ctx.buffer += ctx.text.slice(ctx.i, info.inlineContentStart);
        ctx.i = info.inlineContentStart;
        return true;
    }
...
export const tryConsumeTagStart = (ctx, parseInlineContent) => {
    const info = readTagStartInfo(ctx.text, ctx.i);
    if (!info)
        return false;
    flushBuffer(ctx);
    if (tryConsumeDepthLimitedTag(ctx, info))
        return true;
    const inlineEnd = findInlineClose(ctx.text, info.inlineContentStart);
    if (tryConsumeComplexTag(ctx, info, inlineEnd, parseInlineContent))
        return true;
    return tryConsumeInlineTag(ctx, info, inlineEnd);
};
export const finalizeClosedNode = (ctx, node) => {
    ...
    getCurrentTokens(ctx).push(handler?.inline
...
export const tryConsumeTagClose = (ctx) => {
    const { endTag } = getSyntax();
    if (!ctx.text.startsWith(endTag, ctx.i))
        return false;
...
export const tryConsumeEscape = (ctx) => {
